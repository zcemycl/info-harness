"""Search ClinicalTrials.gov API v2 by study id (OrgStudyId / SecondaryId / NCT)."""
...
def run_search_ctgov_by_id(
    study_id: str,
    *,
    page_size: int = 10,
) -> list[CtgovStudyHit]:
    """GET /studies with query.id; return projected hits."""
    q = study_id.strip()
    if not q:
    ...
    data = ctgov_request_json(
        "/studies",
        params={"query.id": q, "pageSize": size, "fields": _FIELDS},
    )

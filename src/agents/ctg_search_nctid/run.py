"""Run the CTG NCT-id search worker agent."""
...
from agents.ctg_search_nctid.tools import nctid_worker_tools
...
def run_ctg_search_nctid(
    brief: str,
    *,
    max_turns: int = 8,
    run_id: str | None = None,
) -> AgentAnswer:
    """Run the NCT-id worker; return + persist an answer for human/next agent."""
    rid = run_id or uuid.uuid4().hex[:12]
    with trace_span("worker", "ctg_search_nctid", max_turns=max_turns):
        result = run_tool_agent(
            model=chat_model(),
            tools=nctid_worker_tools(),
            system=load_prompt("ctg", "search_nctid_worker.md"),
            user=brief,
            max_turns=max_turns,
        )
        ...
        return write_agent_answer(
            ...
            name_prefix=f"{rid}/ctg_search_nctid",

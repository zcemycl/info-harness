"""Build + persist a PEWE stage answer for the next stage / human."""
...
def emit_stage_answer(
    *,
    ...
    """Write ``data/diary/{run_id}/fda/loop-{n}/{agent}/answer-*.json``."""
    ...
    return write_agent_answer(
        entry,
        name_prefix=f"{run_id}/fda/loop-{loop}/{agent}",

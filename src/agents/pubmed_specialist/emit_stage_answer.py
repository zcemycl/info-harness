"""Build + persist a PEWE stage answer for PubMed specialist stages."""
...
def emit_stage_answer(
    *,
    ...
    """Write diary answer under ``{run_id}/pubmed/loop-{n}/{agent}/``."""
    ...
    return write_agent_answer(
        entry,
        name_prefix=f"{run_id}/pubmed/loop-{loop}/{agent}",

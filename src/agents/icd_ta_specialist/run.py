"""Run the ICD therapeutic-area specialist PEWE loop."""
...
from agents.icd_ta_specialist.pewe_stages import (
    run_evaluator_stage,
    run_executor_stage,
    run_planner_stage,
    run_writer_stage,
)
from agents.icd_ta_specialist.synthesize_answer import synthesize_icd_answer
...
from tools.diary.write_icd_specialist_result import write_icd_specialist_result
from tools.trace_call import trace_info, trace_span


def run_icd_ta_specialist(
    brief: str,
    *,
    max_loops: int | None = None,
    run_id: str | None = None,
) -> IcdSpecialistResult:
    """PEWE loop dispatching ICD TA search_therapeutic_area plans."""
    limit = max_loops or int(os.getenv("ICD_TA_SPECIALIST_MAX_LOOPS", "3"))
    ...
    with trace_span("agent", "icd_ta_specialist", run_id=rid, max_loops=limit):
    ...
    path = write_icd_specialist_result(result, name_prefix=f"{rid}/icd")
...
def _finalize_answer(
    ...
    with trace_span("stage", "synthesize"):
        text = synthesize_icd_answer(brief, evidence).strip()

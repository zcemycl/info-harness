"""Build + persist a PEWE stage answer under the ICD diary tree."""
...
def emit_stage_answer(
    *,
    ...
    """Write ``data/diary/{run_id}/icd/loop-{n}/{agent}/answer-*.json``."""
    ...
    return write_agent_answer(
        entry,
        name_prefix=f"{run_id}/icd/loop-{loop}/{agent}",

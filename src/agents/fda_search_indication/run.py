"""Run the FDA indication search worker agent."""
...
from agents.fda_search_indication.tools import indication_worker_tools
...
def run_fda_search_indication(
    brief: str,
    *,
    max_turns: int = 8,
    run_id: str | None = None,
) -> AgentAnswer:
    """Run the indication worker; return + persist an answer for human/next agent."""
    rid = run_id or uuid.uuid4().hex[:12]
    with trace_span("worker", "fda_search_indication", max_turns=max_turns):
        result = run_tool_agent(
            model=chat_model(),
            tools=indication_worker_tools(),
            system=load_prompt("fda", "search_indication_worker.md"),
            user=brief,
            max_turns=max_turns,
        )
        ...
        return write_agent_answer(
            ...
            name_prefix=f"{rid}/fda_search_indication",

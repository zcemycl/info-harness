"""Thin CLI: run the CTG specialist PEWE loop."""
...
def ctg_specialist(
    ...
    max_loops: int | None = typer.Option(
        None, "--max-loops", help="Override CTG_SPECIALIST_MAX_LOOPS"
    ),
    ...
    """Run CTG specialist planner→executor→writer→evaluator loop."""
    try:
        result = run_ctg_specialist_pipeline(brief, max_loops=max_loops, run_id=run_id)

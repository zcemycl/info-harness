"""Register one Typer command per FdaLabel attr for tradename search."""
...
def register_tradename_attr_commands(app: typer.Typer) -> None:
...
def _make_command(attr: FdaAttrName, search: object) -> Callable[..., None]:
    ...
    command.__doc__ = f"Search by tradename; return {attr.value} page."
    return command

"""Register one Typer command per FdaLabel attr for therapeutic-area search."""
...
def register_therapeutic_area_attr_commands(app: typer.Typer) -> None:
...
def _make_command(attr: FdaAttrName, search: object) -> Callable[..., None]:
    def command(
        ta_description: str = typer.Argument(
            ..., metavar="TA_DESCRIPTION", help="Therapeutic area text"
        ),
    ...
    command.__doc__ = f"Search by therapeutic area; return {attr.value} page."
    return command
